...
<header id="vf-header" class="vf-header" >
	<?php do_action('storepress_top_header_data'); ?>
		...
		<div id="navigation-wrapper" >
			<div class="navigation-middle <?php echo esc_attr(storepress_sticky_menu()); ?>">
								...
								<!-- <div class="col-lg-3 col-12 my-auto">
									...
									<div class="col-12 my-auto">
										<nav class="navbar-area">
											...
											<!-- <ul class="" name="product_cat">
													<li value=""><?php esc_html_e('All Categories', 'storepress'); ?></li> 
													<?php
													$categories = get_categories('taxonomy=product_cat');
													foreach ($categories as $category) {
														// $option = '<option value="' . esc_attr($category->category_nicename) . '">';
														$option = '<li><a href="' . get_term_link($category->term_id) . '">';
														$option .= esc_html($category->cat_name);
														$option .= ' (' . absint($category->category_count) . ')';
														$option .= '</a></li>';
														echo $option; // WPCS: XSS OK.
